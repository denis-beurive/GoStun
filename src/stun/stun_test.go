...
func test_go_conv(in_test *testing.T) {
	...
	u16 = uint16(1)
	if u16 != 50 {
		in_test.Errorf("Invalid unisigned int. Got %d, expected %d ", u16, i)
	}
	...
	u16 = uint16(i)
	if u16 != 0 {
		in_test.Errorf("Invalid unisigned int. Got %d, expected %d ", u16, 0)
	}

	i = -1
	u16 = uint16(i)
	if u16 != 65535 {
		in_test.Errorf("Invalid unisigned int. Got %d, expected %d ", u16, 65535)
	}
}
...
func test_fingerprint(in_test *testing.T) {
	...
	res := __stunCrc32(b)
	if 0xc8fb0b4c != res {
		in_test.Errorf("Invalid fingerptint.")
	}
}
